...
public class DatabaseSeedingService {
    ...
    @Transactional
    public void seedDefaultAdminAndRoles(
            String adminEmail,
        ...
        if (adminPassword == null || adminPassword.trim().length() < 12 || adminPassword.trim().length() > 24) {
            throw new IllegalArgumentException(
                    "CRITICAL: Default Admin password in properties must be between 12 and 24 characters!"
            );
        }
        ...
        UserAccount admin = userRepository.findByEmailCanonical(normalizedAdminEmail);
        ...
        if (admin == null) {
            ...
            // REPAIR CASE: USER EXISTS BUT SECURITY PROFILE IS MISSING
            if (admin.getSecurity() == null) {
                ...
                log.warn("DATABASE REPAIR: Missing UserSecurity profile recreated for admin.");
            }
